...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      // 마운트되었을 때 콜링하여 현재의 라우트 정보를 이용해 이전, 다음 ID 획득
      ...
      onBeforeRouteUpdate((to) => {
        // 컴포넌트가 마운트 될 때, 라우트가 변경될 때(이 때 사용할 수 있는 훅 onBeforeRouteUpdate()) 영상 ID정보(현재, 이전, 다음 포함)를 변경해줘야 하므로 영상정보를 변경해 줄 함수 getNavId() 만들어두기
        getNavId(to);
      });
